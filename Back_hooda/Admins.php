...
// data
$query = "SELECT id,nom FROM utilisateurs WHERE (UPPER(nom) LIKE '%" . $filter . "%') LIMIT $debut,$nbr_element_page";
$affichage = mysqli_query($connect, $query) or die(mysqli_error($connect));
while ($ligne = mysqli_fetch_array($affichage)) {
    $admin = array(
        "idAdmin" => $ligne['id'],
        "nomComplet" => $ligne['nom']
    );
    array_push($retour, $admin);
}
// number data
$query = "SELECT COUNT(*) FROM utilisateurs WHERE (UPPER(nom) LIKE '%" . $filter . "%')";
$affichage = mysqli_query($connect, $query) or die(mysqli_error($connect));
