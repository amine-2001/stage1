...
class Clients extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            columns: [
                ...
                {
                    name: 'Desactiver',
                    cell: row => <button className='buttonSupprimer' onClick={() => this.toggletwo(row.idClient)}><i style={{ color: 'white' }} className="fas fa-trash-alt"></i></button>,
                    ignoreRowClick: true,
                    allowOverflow: true,
                    button: true,
                }
    ...
    componentDidMount() {
        jQuery.ajax({
            url: process.env.REACT_APP_API_URL + 'Back_hooda/Clients.php',
            type: 'POST',
            data: {
                query: '',
    ...
    deleteClient() {
        jQuery.ajax({
            url: process.env.REACT_APP_API_URL + "Back_hooda/DeleteClient.php?id=" + this.state.idClient,
            type: 'POST',
            dataType: 'json',
            success: (code_html, statut) => {
                this.setState({
                        ...
                        <div className="col-sm-12">
                            <div className="white-box">
                                <div style={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center', marginBottom: '10px', marginRight: '20px' }}>
                                    <Link to="/AjoutClient" > <button style={{ background: '#3A3F51', border: '1px solid #3A3F51', borderRadius: '5px' }} type="button" className="btn btn-info"> <i className="fas fa-plus"></i>Ajout client</button></Link>
                                </div>
