...
// data
$query = "SELECT * FROM clients WHERE (raisonsocial LIKE '%" . $filter . "%' OR responsable LIKE '%" . $filter . "%' OR telephone LIKE '%" . $filter . "%' ) LIMIT $debut,$nbr_element_page";
...
// number data
$query = "SELECT COUNT(*) FROM clients WHERE (raisonsocial LIKE '%" . $filter . "%' OR responsable LIKE '%" . $filter . "%' OR telephone LIKE '%" . $filter . "%' )";
$affichage = mysqli_query($connect, $query) or die(mysqli_error($connect));
